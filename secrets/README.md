...
## 主配置（与 `scripts/process_email_qa.py`、`scripts/scrub_markdown_pii.py`、`scripts/clean_qa_jsonl.py` 共用）
...
模型名统一用环境变量 **`OPENAI_MODEL`**（各脚本内有默认值，与 `scripts/process_email_qa.py` 一致）。

### 脱敏脚本 `scripts/scrub_markdown_pii.py` 的默认行为
